...
import descripcionJson from '../../assets/jsons/descripcion_personal.json';
...
export class InicioComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.typeWriter();
    this.animacionesService
      .getAnimaciones()
      .subscribe((animaciones) => (this.animaciones = animaciones));
  }
  ...
  typeWriter(): void {
    if (this.charIndex < this.descripcionPersonal.length) {
      ...
      const charToWrite = this.descripcionPersonal.charAt(this.charIndex);
      if (charToWrite === '<') {
        this.typingSpeed = 0;
      } else if (charToWrite === '>') {
        if (++this.closeHtmlCount == 2) {
          this.typingSpeed = this.defaultTypingSpeed;
          this.closeHtmlCount = 0;
        }
      }
